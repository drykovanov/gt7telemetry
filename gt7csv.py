...
while True:
	try:
		...
		if len(ddata) > 0 and struct.unpack('i', ddata[0x70:0x70+4])[0] > pktid:
			...
			posX = struct.unpack('f', ddata[0x04:0x04+4])[0]
			posY = struct.unpack('f', ddata[0x08:0x08+4])[0]
			posZ = struct.unpack('f', ddata[0x0C:0x0C+4])[0]
			...
			printAt('{:11.4f}'.format(posX), 30, 23)					# pos X
			printAt('{:11.4f}'.format(posY), 31, 23)					# pos Y
			printAt('{:11.4f}'.format(posZ), 32, 23)					# pos Z
